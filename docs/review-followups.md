...
## 2. Ship blind validation evidence for the judgment layer (harness built in 0.5.15; dataset and run open)

The linter now has real test coverage, but the seven-dimension judgment pass has none that is public. The claim that a run gives a fair read of whether a draft sounds like its writer rests on a private single-writer predecessor. The protocol for closing this is written in `docs/blind-eval.md`: for several writers, hold out authentic prose, a deliberately flattened rewrite, an unusual-but-authentic piece, another writer's prose, and an override sample, then report where Pherkad passes, revises, rewrites, and gets it wrong, with the failures quoted. The harness for it is `eval/detect.py` (0.5.15): the case types, the shuffled and linter-only controls, repeats, the reader reference, and per-rule precision, with a preregistration file the scorer checks. What remains is building the dataset from real samples and running it. This is the single strongest thing that would raise the tool from advisory to dependable, and the biggest lift.

## 3. Profile holdout check (done, hardened in 0.4.6)

Added to `profile_builder.md` as Step 4b, then repaired after review: the holdout is chosen and isolated before Step 2 reads any sample (a read sample cannot be made unseen); a failed holdout is spent, so a profile changed to fix it is confirmed on a new untouched sample; the flattened rewrite is produced by an editor or model blind to the profile, preserving facts and order within 10 percent length; and the impostor check uses several closely matched writers. It is a per-writer sanity check, not method-level evidence; that is item 2.
