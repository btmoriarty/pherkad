...
The generic half of Pherkad's diagnostic: constructions and phrases that often appear in AI-assisted drafts and can flatten a writer's voice, regardless of whose voice is being validated. No item proves how a passage was written; it marks a pattern worth a look. Personal markers live in the user's `Voice_Profile.md`, not here.
...
- **Informal registers between people** (a casual message, a note to a friend) where a relaxed register is appropriate; the seven-dimension audit is for drafts intended as public posts, papers, and finished prose. An AI assistant's reply to the writer is not exempt: it is the `assistant-chat` surface, checked mechanically by `tools/replycheck.py` and judged in the skill's quick mode against the bans, the honest-X family, and the chat-only rules, with no positive-register expectation.
