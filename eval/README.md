...
1. Cases live in the writer's directory: `holdout/` (authentic held-out pieces; name one `atypical-*.md`), `flattened/<holdout>.<k>.md` (independent flattenings of a held-out piece, facts kept), `impostors/` (other writers, matched on register and topic), `override/` (authentic pieces that use an allowed habit). `python3 study.py flatten <writer> --runner "<command>" --k 2` writes the flattenings through a runner that never sees the profile, holds each to within 10 percent of its source length, and records the runner, model, and hashes in a `.meta.json` beside each one; use a different model family from the judge, and read every flattening against its source before planning. `python3 study.py plan <run> --task detect --writers a,b [--repeats 3]` plans every case under five conditions: `correct`, `wrong`, `shuffled` (the writer's own profile with its lines scrambled, written by plan), `none`, and `linter` (the mechanical verdict alone, the floor). It writes `prereg.md`; fill every TODO before collecting a verdict.
...
Two runners ship in `eval/runners/`. `claude.sh` calls the Claude CLI isolated from the operator's own setup (`--setting-sources ""` so no hooks fire, no CLAUDE.md voice rules load, no tools, no saved session; `CLAUDE_MODEL` picks the model), which matters because the generic arm is the control and must not see the rules through the back door. `codex.sh` calls the Codex binary the same way (`CODEX_MODEL`, `CODEX_BIN`) and prints only the final message; use it as the flattener so the flattener and the judge are different families.
